...
class CFGBuilder:
    ...
    def detect_malicious_patterns(self, cfg: CFGNode):
        self._detect_code_obfuscation(cfg)
        self._find_sensitive_api_calls(cfg)
        self._analyze_loop_structures(cfg)
    ...
    def _detect_code_obfuscation(self, node: CFGNode):
        if node.ast_node and node.ast_node.type == 'CallExpression':
            ...
            if callee in ['Function', 'eval', 'setTimeout']:
                args = [arg.source() for arg in node.ast_node.arguments]
                if any(len(arg) > 100 and re.search(r'[\\x\\u]', arg) for arg in args):
...
class TaintAnalyzer:
    ...
    def _get_priority(self, node: CFGNode) -> int:
        ...
        try:
            if node.is_sink:
                return 100
            if node.ast_node and hasattr(node.ast_node, 'type') and node.ast_node.type == 'CallExpression':
                return 50
            return len(node.tainted_vars) if hasattr(node, 'tainted_vars') else 0
    ...
    def _mark_tainted(self, node: CFGNode):
        ...
        try:
            args = self._get_arguments(node.ast_node)
            for arg in args:
                if arg and hasattr(arg, 'type') and arg.type == 'Identifier' and hasattr(arg, 'name'):
                    node.tainted_vars.add(arg.name)
                    source = node.ast_node.source() if hasattr(node.ast_node, 'source') else 'unknown'
                    node.taint_labels[arg.name].add(TaintLabel(source=source))
        except AttributeError as e:
            print(f"Taint marking failed: {str(e)}")
